...
#define kDefaultRefreshInterval 0.5 //默认0.1秒刷新一次
...
if(operation!=noErr) { \
[self postAErrorWithErrorCode:XFAudioMeterObserverErrorCodeAboutQueue andDescription:error]; \
return; \
}   \

@implementation XFLevelMeterState
@end

@interface XFAudioMeterObserver()
...
@implementation XFAudioMeterObserver
...
- (void)refresh {
    ...
    for (int i=0; i<self.channelCount; i++) {
        XFLevelMeterState *state = [[XFLevelMeterState alloc] init];
...
- (void)postAErrorWithErrorCode:(XFAudioMeterObserverErrorCode)code andDescription:(NSString*)description {
    self.audioQueue = nil;
    NSLog(@"监控音频队列光谱发生错误");
...
+ (Float32)volumeForLevelMeterStates:(NSArray*)levelMeterStates {
    ...
    for (NSInteger i = 0; i < levelMeterStates.count; i++) {
        averagePowerOfChannels += ((XFLevelMeterState*)levelMeterStates[i]).mAveragePower;
    }
    //获取音量百分比，姑且这么叫吧
